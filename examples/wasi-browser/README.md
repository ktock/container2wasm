...
- cons: This runs containers on emulators with interpreter approach with a single thread so the execution can be slower than the emscripten example which uses QEMU with JIT(TCG) with multi thread enabled.
