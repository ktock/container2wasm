...
all: c2w c2w-net d2oci

build: c2w c2w-net d2oci
...
d2oci:
	CGO_ENABLED=0 go build -o $(PREFIX)/d2oci $(GO_LD_FLAGS) $(GO_BUILDTAGS) -v ./cmd/d2oci
...
install:
	...
	install -D -m 755 $(PREFIX)/d2oci $(CMD_DESTDIR)/bin

artifacts: clean
	GOOS=linux GOARCH=amd64 make c2w c2w-net d2oci
	tar -C $(PREFIX) --owner=0 --group=0 -zcvf $(PREFIX)/container2wasm-$(VERSION)-linux-amd64.tar.gz c2w c2w-net d2oci

	GOOS=linux GOARCH=arm64 make c2w c2w-net d2oci
	tar -C $(PREFIX) --owner=0 --group=0 -zcvf $(PREFIX)/container2wasm-$(VERSION)-linux-arm64.tar.gz c2w c2w-net d2oci

	rm -f $(PREFIX)/c2w $(PREFIX)/c2w-net $(PREFIX)/d2oci
